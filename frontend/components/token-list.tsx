...
interface Token {
  ...
  description: string
  mint?: string // on-chain mint (base58)
}
...
export function TokenList() {
  ...
  const { supportedTokens, loading } = useStaking()

  const tokensToDisplay: Token[] = useMemo(() => {
    if (!supportedTokens?.length) return mockTokens
    return supportedTokens.map((t, idx) => ({
      id: String(idx + 1),
      name: t.name || t.symbol || "Token",
      symbol: t.symbol || "TOKEN",
      logo: "/generic-token-logo.png",
      apy: t.apy ?? 10,
      totalStaked: "—",
      totalStakedUSD: "—",
      activeStakers: 0,
      ...
      isVerified: true,
      trend: "stable",
      description: "On-chain supported token from staking pool.",
  ...
  return (
    <>
      {loading && <div className="mb-4 text-sm text-muted-foreground">Loading supported tokens from program…</div>}
            ...
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <img
                    src={token.logo || "/placeholder.svg?height=40&width=40&query=token%20logo"}
                    alt={token.name}
